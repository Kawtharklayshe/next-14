...
import { GetFieldExperimentsListItems } from "../services/endpoints";
import {
  checkLoadImages,
...
import VideoCard from "../components/pages/FieldExperiments/videoCard";
import AutoPagination from "../components/Shared/customPagination";
import useStyles from "../components/pages/FieldExperiments/style";

function FieldExperiments({ initialData, headerType, theme }) {
  ...
  const { data, error } = useSWR(
    GetFieldExperimentsListItems(currentPage, 10),
    autherizedFetcher
  );
...
export async function getStaticProps({ locale }) {
  const res = await getFetch(
    GetFieldExperimentsListItems(1, 10),
    process.env.NEXT_PUBLIC_MERCHANT,
    locale
  );
